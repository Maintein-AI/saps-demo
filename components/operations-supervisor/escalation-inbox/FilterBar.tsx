...
const filters = [
  { key: "escalationId", label: "Escalation ID", options: ["All", "ESC-2026-00101", "ESC-2026-00102", "ESC-2026-00103", "ESC-2026-00104", "ESC-2026-00105"] },
  { key: "sourceModule", label: "Source Module", options: ["All", "Warehouse", "Gate Entry", "Finance", "Excise", "Planner", "Lifter"] },
  { key: "awb", label: "AWB #", options: ["All", "214-45678901", "157-90811223", "074-88219033", "117-55443321", "117-98765432"] },
  { key: "issueType", label: "Issue Type", options: ["All", "Damage", "Shortage", "Missing Docs", "Customs Hold", "Payment Hold", "Gate Mismatch", "RFID Mismatch", "Rack Conflict", "Cold-chain Anomaly", "Long-stay Alert"] },
  { key: "priority", label: "Priority", options: ["All", "Low", "Medium", "High", "Critical"] },
  { key: "assignedTo", label: "Assigned To", options: ["All", "Imran Ali", "Nadeem Shah", "Sana Khan", "Kamran Hussain", "Faisal Qureshi"] },
  { key: "status", label: "Status", options: ["All", "Open", "Awaiting Decision", "Reassigned", "Approved", "Rejected", "Closed"] },
  { key: "dateRange", label: "Date Range", options: ["Today", "Yesterday", "Last 7 Days", "This Week", "Custom"] },
];

export default function FilterBar() {
  const [expanded, setExpanded] = useState(false);
  const [active, setActive] = useState<Record<string, string>>({ dateRange: "Today", status: "All" });
  ...
  return (
    <div className="rounded-[16px] border border-[#E2E8F0] bg-white p-4 shadow-sm">
      ...
      {expanded && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8 gap-3 mt-4">
          {filters.map((f) => (
            <div key={f.key}>
