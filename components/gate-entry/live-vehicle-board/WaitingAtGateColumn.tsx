...
import { Clock, Phone, Eye, PauseCircle, CheckCircle2, AlertTriangle } from "lucide-react";
...
const vehicles = [
  ...
  },
  {
    ...
    queueTime: "3m",
    verification: "Hold",
    holdReason: "Missing ID card",
  },
];

interface WaitingAtGateColumnProps {
  onAction: (action: string, vehicle: string) => void;
}

export default function WaitingAtGateColumn({ onAction }: WaitingAtGateColumnProps) {
  return (
    <div className="rounded-[16px] border border-[#E2E8F0] bg-white p-5 shadow-sm">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-xl bg-[#DBEAFE] flex items-center justify-center">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1B4F8B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2" />
            <circle cx="7" cy="17" r="2" />
            <circle cx="17" cy="17" r="2" />
          </svg>
        </div>
        <h2 className="text-[18px] font-bold text-[#0F172A]">Waiting / At Gate</h2>
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <GateStatusBadge status={v.verification} />
                  </div>
              ...
              <div className="text-right">
                ...
                <p className="text-[11px] text-[#94A3B8]">In queue</p>
              </div>
            </div>

            {v.holdReason && (
              <div className="flex items-center gap-2 rounded-lg bg-[#FEF2F2] border border-[#FEE2E2] px-3 py-2 mb-3">
                <AlertTriangle size={14} className="text-[#DC2626] flex-shrink-0" />
                <span className="text-[13px] font-semibold text-[#DC2626]">Hold: {v.holdReason}</span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              ...
              <div className="rounded-lg bg-[#F8FAFC] px-3 py-2">
                <p className="text-[11px] text-[#94A3B8] uppercase font-semibold">Status</p>
                <p className="text-[13px] font-semibold text-[#0F172A]">{v.verification}</p>
              </div>
            </div>
