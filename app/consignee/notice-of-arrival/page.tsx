...
import KPIStrip from "@/components/consignee/notice-of-arrival/KPIStrip";
import FilterBar from "@/components/consignee/notice-of-arrival/FilterBar";
import NOAInbox from "@/components/consignee/notice-of-arrival/NOAInbox";
import EmptyState from "@/components/EmptyState";
import ErrorState from "@/components/ErrorState";
import { useToast } from "@/components/ToastContext";
import { RefreshCw, ArrowRight, AlertTriangle, Bell } from "lucide-react";

export default function NoticeOfArrivalPage() {
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      addToast("Notices refreshed.", "success");
    }, 1000);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <div className="flex items-center gap-3 mt-3">
            <h1 className="text-[24px] font-bold text-[#0F172A]">Notice of Arrival</h1>
            <ScopeBadge type="exc" />
          </div>
        </div>
      ...
      {error && (
        <ErrorState
          ...
          message="Could not retrieve Notice of Arrival data. Please try again."
      ...
      {empty && !error && !loading && (
        <EmptyState
          title="No Notice of Arrival messages found."
          description="No arrival notices have been received for your shipments yet. New notices will appear here."
          icon={<Bell size={28} className="text-[#94A3B8]" />}
          actionLabel="Refresh"
