...
import DetailDrawer from "@/components/operations-supervisor/escalation-inbox/DetailDrawer";
import Timeline from "@/components/operations-supervisor/escalation-inbox/Timeline";
import EmptyState from "@/components/EmptyState";
import ErrorState from "@/components/ErrorState";
import { useToast } from "@/components/ToastContext";
import { RefreshCw, ArrowRight, AlertTriangle, Inbox } from "lucide-react";
...
export default function EscalationInboxPage() {
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      addToast("Escalation inbox refreshed.", "success");
    }, 1200);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <ErrorState title="Failed to load escalation inbox" message="Could not retrieve escalation data. Please try again." onRetry={handleRefresh} />
      )}

      {empty && !error && !loading && (
        <EmptyState title="No escalations pending" description="No escalations are currently pending for your review." icon={<Inbox size={28} className="text-[#94A3B8]" />} actionLabel="Refresh" onAction={handleRefresh} />
      )}
      ...
      {!loading && !error && !empty && (
        <div className="space-y-6">
          <KPIStrip />
          <FilterBar />
          <EscalationTable onViewDetail={handleViewDetail} />
          <Timeline />
        </div>
      )}

      <DetailDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} escalationId={selectedId} />
    </div>
  );
}