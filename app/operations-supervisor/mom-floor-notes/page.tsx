...
import KPIStrip from "@/components/operations-supervisor/mom-floor-notes/KPIStrip";
import FilterBar from "@/components/operations-supervisor/mom-floor-notes/FilterBar";
import NotesList from "@/components/operations-supervisor/mom-floor-notes/NotesList";
import NoteEditor from "@/components/operations-supervisor/mom-floor-notes/NoteEditor";
import FollowUpTable from "@/components/operations-supervisor/mom-floor-notes/FollowUpTable";
import EmptyState from "@/components/EmptyState";
import ErrorState from "@/components/ErrorState";
import { useToast } from "@/components/ToastContext";
import { RefreshCw, ArrowRight, AlertTriangle, FileText } from "lucide-react";

export default function MomFloorNotesPage() {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  const [empty, setEmpty] = useState(false);
  const [selectedNote, setSelectedNote] = useState("NOTE-2026-0061");
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      addToast("Notes refreshed.", "success");
    }, 1000);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <Breadcrumb items={[
            ...
            { label: "MoM / Floor Notes" }
          ]} />
          <div className="flex items-center gap-3 mt-3">
            <h1 className="text-[24px] font-bold text-[#0F172A]">MoM / Floor Notes</h1>
          </div>
        </div>
      ...
      {error && (
        <ErrorState title="Failed to load notes" message="Could not retrieve floor notes. Please try again." onRetry={handleRefresh} />
      )}

      {empty && !error && !loading && (
        <EmptyState title="No floor notes found" description="No floor notes or meeting minutes are available for this period." icon={<FileText size={28} className="text-[#94A3B8]" />} actionLabel="Refresh" onAction={handleRefresh} />
      )}
      ...
      {!loading && !error && !empty && (
        <div className="space-y-6">
          ...
          <FilterBar />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <NotesList onSelectNote={setSelectedNote} selectedId={selectedNote} searchTerm={searchTerm} />
